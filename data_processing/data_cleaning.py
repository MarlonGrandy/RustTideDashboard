...
def clean_URI_data():
    ...
    URI_data = URI_data.resample('W').max()
    URI_data['Lat'] = 41.5722
    URI_data['Long'] = -71.3944
    return URI_data

# Function to clean WHOI data
def clean_WHOI_data():
    ...
    WHOI_data['Margalefidinium polykrikoides'] = WHOI_data['Margalefidinium polykrikoides'].astype(float)
    ...
    WHOI_data = WHOI_data.resample('W').max()
    WHOI_data['Lat'] = 41.49224
    WHOI_data['Long'] = -71.41896
    return WHOI_data

# Function to clean meteorological data
def clean_met_data():
    ...
    MaxWSpd = met_data['MaxWSpd'].resample('W').max()
...
# Function to clean water quality data
def clean_WQ_data():
    WQ_data = pd.read_csv(
...
# Function to create a consolidated dataframe for the dashboard
def dashbaord_df():
